...
if(!$filter){
	$queries[] = "SELECT `title`, title_sort, `subcategory`, `type`, index_data 
		FROM pages WHERE redirect_to='' AND (`title` LIKE '%$q%' OR `keywords` LIKE '%$q%') 
		ORDER BY title_sort LIMIT 30";
} else {
	$tables = array(
		"categories" => "SELECT `title`, title_sort, `subcategory`, `type`, index_data FROM pages WHERE `type` = 'category' AND `redirect_to` = '' AND (`title` LIKE '%$q%' OR `keywords` LIKE '%$q%') ORDER BY `title` LIMIT 100",
		"games" => "SELECT `title`, title_sort, `subcategory`, `type`, index_data FROM pages WHERE `type` = 'game' AND `redirect_to` = '' AND (`title` LIKE '%$q%' OR `keywords` LIKE '%$q%') ORDER BY `title` LIMIT 100",
		"people" => "SELECT `title`, title_sort, `subcategory`, `type`, index_data FROM pages WHERE `type` = 'person' AND `redirect_to` = '' AND (`title` LIKE '%$q%' OR `keywords` LIKE '%$q%') ORDER BY `title` LIMIT 100",
		"characters" => "SELECT `title`, title_sort, `subcategory`, `type`, index_data FROM pages_links LEFT JOIN pages ON (pages_links.from_pgid = pages.pgid) WHERE (`to` = 'Game character') AND `namespace` = 'Category' AND `redirect_to` = '' AND (`title` LIKE '%$q%' OR `keywords` LIKE '%$q%') ORDER BY `title`",
		"locations" => "SELECT `title`, title_sort, `subcategory`, `type`, index_data FROM pages_links LEFT JOIN pages ON (pages_links.from_pgid = pages.pgid) WHERE (`to` = 'Game location') AND `namespace` = 'Category' AND `redirect_to` = '' AND (`title` LIKE '%$q%' OR `keywords` LIKE '%$q%') ORDER BY `title`",
		"publishers" => "SELECT `title`, title_sort, `subcategory`, `type`, index_data FROM pages_links LEFT JOIN pages ON (pages_links.from_pgid = pages.pgid) WHERE (`to` = 'Game publisher') AND `namespace` = 'Category' AND `redirect_to` = '' AND (`title` LIKE '%$q%' OR `keywords` LIKE '%$q%') ORDER BY `title`"
	);
	foreach($tables as $table => $query){
		if(stristr($filter, $table)) $queries[] = $query;
	}
}
...
if(!$a->ret['num_results'] = count($ret)){
	exit;
}

ksort($ret);

foreach($ret as $title_sort => $arr){
	foreach($arr as $sub_arr) $a->ret['results'][] = $sub_arr;
}
