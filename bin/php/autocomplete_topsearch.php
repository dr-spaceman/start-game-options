...
while($row = mysqli_fetch_assoc($res)) {
	...
	$num = "";
	if($num = mysqli_num_rows(mysqli_query($GLOBALS['db']['link'], $query2))) $x.= " &middot; $num game".($num != 1 ? "s" : "");
	$query2 = "SELECT * FROM people WHERE assoc_co LIKE '`".$row['developer']."`'";
	$num = "";
	if($num = mysqli_num_rows(mysqli_query($GLOBALS['db']['link'], $query2))) $x.= " &middot; $num ".($num != 1 ? "people" : "person");
