...
} elseif($sort = $_POST['sort']) {
	sortCollection();
}

/**
 * Add to collection or edit the properties of an item in a user's collection
 * @return JSON Information about the action
 */
function manageCollectionItem() {
	...
	// Determine if this is a new item or modification
	...
	if($row = mysqli_fetch_assoc(mysqli_query($GLOBALS['db']['link'], $q))){
		// Modify item; Get sort position to notify front end for DOM manipulation
		$sort = $row['sort'];
	} else {
	...
	if(mysqli_query($GLOBALS['db']['link'], $q)){
		...
		if($_POST['return_new_shelf_item']) {
			...
			$shelf_item_properties['img'] = new Img($in['img_name']);
			$shelfitem = new ShelfItem($shelf_item_properties);
			$a->ret['formatted'] = $shelfitem->output();
			// How to do this? This will almost certainly conflict in the DOM
			$a->ret['shelf_id'] = $shelfitem->id;
			/* OLD
			$shelf->type = "game";
...
function sortCollection() {
	...
	if(!$sort = $_POST['sort']) $a->kill('No sort data could be read.');

	$sort = str_replace("shelf-item-id-", "", $sort);
	$i = 0;
	foreach(explode(",", $sort) as $id){
		$id = (int) $id;
		$q = "UPDATE collection SET `sort` = '".++$i."' WHERE id = '$id' AND usrid = '$usrid' LIMIT 1";
...
/**
 * Output the form to add/modify a collection item
 * @param  string $title Title of an item to collect, eg. game or album
 * @return string        Output of the form; False if couldn't compile (Eg. if user data not found)
 */
function collectionForm($title) {

	global $usrid, $pf_regions, $pf_shorthand;

	include_once $_SERVER['DOCUMENT_ROOT']."/bin/php/currencies.include.php";

	if(!$usrid) return false;
	$user = new user($usrid);
	if($user->notfound) return false;
	
	// Check if the item is in collection; If found, assign collection info to $in
	$in_collection = false;
	...
	if ($is_album) {
		...
		$shelfitem_properties = $album->row;
		$shelfitem_properties['type'] = "album";
		$shelfitem_properties['filename'] = $album->coverimg;
		$shelfitem = new shelfItem($shelfitem_properties);
		
		$o_shelf = $shelfitem->output();
		...
		foreach($shelf_rows as $row){
			...
			$shelfitem_props = $row;
			$shelfitem_props['type'] = "game";
			$shelfitem_props['img'] = new Img($img_name);
			$shelfitem = new ShelfItem($shelfitem_props);
			...
			$js = "$(this).addClass('on').siblings().removeClass('on'); collection.selectBox($('#shelf-item-id-".$shelfitem->id."')); shelf.traverse($('#pgop-form-collection .shelf'), {position:".$num_shelfitems."}, '', 300);";
			$o_title = ($o_title ? $o_title." &middot; " : "").$row['platform']." &middot; ".$pf_regions_x[$row['region']]." &middot; ".substr($row['release_date'], 0, 4);
			$shelf_select.= '<a onclick="'.$js.'" title="'.htmlsc($o_title).'">'.($img_name ? '<img src="/image.php?img_name='.$img_name.'&showimage=true&size=tn"/>' : '').'</a>';
	...
	if(!$num_shelfitems){
		$shelfitem_props = $rows['primary'];
		$shelfitem_props['href'] = "javascript:collection.selectBox($('#shelf-item-id-".$row['id']."'))";
		$shelfitem_props['type'] = "game";
	...
	if($is_album){
		$heading = ($in_collection ? 'Edit' : 'Add') . ' your copy of <b><a href="'.$album->url.'" target="_blank">'.$album->full_title.'</a></b>';
	} else {
		$heading = $in_collection ? 'Edit your copy of <b><a href="'.$pg->url.'" target="_blank">'.$title.'</a></b>' : 'Which version of <b><a href="'.$pg->url.'" target="_blank">'.$title.'</a></b> would you like to add?';
	}
	...
	<form name="collection-form" class="collection-form" onsubmit="collection.update($(this)); return false;">
		...
		<button type="submit">'.($in_collection ? 'Submit changes' : 'Add to my games').'</button> &nbsp; 
		<a id="collection-rm" class="red" style="'.(!$in_collection ? 'display:none' : '').'">Remove</a>
	</form>
