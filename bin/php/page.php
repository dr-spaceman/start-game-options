...
/*if($usrrank < 1) {
	if(!strstr($_SERVER['SCRIPT_NAME'], "login.php") && !strstr($_SERVER['SCRIPT_NAME'], "retrieve-pass.php") && !strstr($_SERVER['SCRIPT_NAME'], "register.php")){
		include($_SERVER['DOCUMENT_ROOT']."/splash_closed.php");
		exit;
	}
}*/
...
var $called;
var $minimalist;      // if true, minimize page bells & whistles
var $superminimalist; // header fixed to bottom
var $fullwidth;
var $title;
var $dom = array();   // attributes to apply to parts of the page [body, header]
                      // ie $dom['body']['style'][]='font-size:120%;', $dom['body']['class'][]='className'
var $css = array(); 	// list of css filenames
var $freestyle; 			// freestyle css
var $javascript; 			// freestyle and js via html
var $javascripts = array(); //list of js filenames
var $called_sections = array();
var $meta_data;				// [string] add'l meta tags
var $meta = array();  // meta data [$property => $content]
var $fb; 							// connect to facebook
var $first_section;   // attributes to give to the first page section
public $badges;
...
global $html_tag, $usrid, $usrrank, $user;

if(!$user->id) unset($user);
if($usrid && !$user) $user = new user($usrid);

if($this->superminimalist) $this->minimalist = true;

$this->called_sections['header'] = 1;

if(!is_array($this->css)) $this->css = array();
if($this->css){
	foreach($this->css as $src) {
		if($less = substr($src, -5) == ".less") $this->javascripts[] = "/bin/script/less.js";
		if(strpos($print_style, $src) === FALSE) $print_style.= '<link rel="stylesheet'.($less ? '/less' : '').'" type="text/css" href="'.$src.'" media="screen"/>'."\n\t";
	}
}
if($this->freestyle && !strstr($freestyle, '<style')) $print_style.= '<style type="text/css"><!--'.$this->freestyle.'--></style>'."\n\t";
...
if(is_array($this->meta)){
	foreach($this->meta as $property => $content) $this->meta_data.= '<meta property="'.htmlSC($property).'" content="'.htmlSC($content).'"/>'."\n\t";
}

//if($this->fb) $html_tag = substr($html_tag, 0, -1).' xmlns:fb="http://www.facebook.com/2008/fbml" xmlns:og="http://opengraphprotocol.org/schema/">';
...
<head<?=$this->head?>>
	...
	if($this->javascripts){
		...
		foreach($this->javascripts as $src) echo '<script type="text/javascript" src="'.$src.'"></script>'."\n\t";
	}
	?>
	<?=$this->javascript?>
...
/* moved to login loading screen
if($usrid && $_COOKIE['no_oauth'] == '1'){
	?>
	<div class="popmsg" id="popmsg-fblink" onclick="$.cookie('no_oauth', 'ignore', {expires:100, path:'/'})" style="display:none; top:50%; width:380px; margin:-100px 0 0 -190px;">
		<a class="ximg" onclick="$('#popmsg-fblink').fadeOut()">Close</a>
		<div class="container" style="text-align:center; padding:3em; font-size:14px;">
			Connect your account to Facebook or Twitter to share your Videogam.in activity!<br/><br/>
			<a href="/login_fb.php"><img src="/bin/img/fb_connect_160.png" border="0" alt="connect with Facebook"/></a><br/><br/>
			<a href="/bin/php/twitter/connect.php"><img src="/bin/img/twitter_connect_160.png" border="0" alt="connect with Twiter"/></a><br/><br/>
			<a onclick="$('#popmsg-fblink').fadeOut()">I don't have Facebook or Twitter</a>
		</div>
	</div>
	<script>
		$(['/bin/img/fb_connect_160.png', '/bin/img/twitter_connect_160.png']).preload();
		$(window).load(function(){
			$("#popmsg-fblink").fadeOut().delay(1000).fadeIn(500);
		})
	</script>
	<?
}*/
...
function die_($txt=''){ $this->kill($txt); }
