...
$(document).ready(function(){
	...
	if($("#synopsis .text").length) {
		$("#synopsis").hover(function() {
			$("#synopsis-edit").fadeIn();
		}, function() {
			$("#synopsis-edit").fadeOut();
		});
	};
	
	$("#cef-nav li").mouseover(function() {
		$("#cef-nav li.on").animate({ width:'26px' }, { queue:false, duration:500 }).removeClass('on');
		$(this).animate({ width:'248px' }, { queue:false, duration:500 }).addClass('on');
	});
	
	//gameboxes
	var _idnow = $("#gamebox .primary").attr("id").replace(/fs-/, "");
	
	$("#release-data .text").html( $("#gamebox-"+_idnow+" .rel").html() );
	
	$("#gameboxes .tn a").livequery('click', function(e){
		
		if( $(this).attr('href') == "#top" ) { //add game box
			GCtoggle("pub");
			return;
		}
		
		e.preventDefault();
		var _id = $(this).attr('id').replace(/tn-/, '');
		
		$("li#gamebox-"+_id).addClass("on").siblings().removeClass("on");
		
		//lg bog
		var _h = $("#fs-"+_id).height();
		$("#gamebox").animate({ height:_h+"px" });
		$("#fs-"+_idnow).fadeOut(function(){
			$("#fs-"+_id).fadeIn().addClass("on");
		});
		
		//release info
		$("#gamebox-"+_idnow+" .rel").dequeue();
		$("#gamebox-"+_idnow+" .rel").animate({ width:"0" }, function(){
			$("#gamebox-"+_id+" .rel").animate({ width:"400px" }).fadeTo(3000, 1, function() {
	...
	$("#add-game-button").click(function(_e) {
		_e.preventDefault();
		addGame(gid);
	});
	
	$("#gameboxes .rel").hover(function() {
		$(this).dequeue().show();
	}, function() {
		$(this).animate({ width:"0" });
	});
	
	$("input[name='pub-primary']").click(function() {
		$("input[type='hidden'].pub-primary").val('');
		$(this).closest(".form").children("input.pub-primary").val('1');
	});
	
	$("#trivia li.item").each(function() {
		var _h = $(this).children(".fact").height();
		var realh = _h;
		if(_h > 60) $(this).css("height", "56px").css("max-height", "none").children(".fact").prepend('<div class="readon">... <a href="javascript:void(0);" class="arrow-right" onclick="$(this).parent().hide().closest(\'.item\').animate({ height:\''+realh+'px\' }, function(){ $(this).css(\'height\', \'auto\'); });">Read on</a> </div>');
		else $(this).css("max-height", "none");
	}).hover(function() {
		$(this).addClass("showinfo");
	}, function() {
		$(this).removeClass("showinfo");
	});
	$("#trivia .info").click(function(act) {
		act.preventDefault();
...
function relOn(_id) {
	$("#gamebox-"+_id+" .rel").dequeue().animate({ width:"400px" });
}
