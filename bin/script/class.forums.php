...
class forum {
	...
	function outputPost($row, $ret=FALSE, $justposted=FALSE) {
		...
		<li id="p<?=$row['pid']?>" class="postitem st-<?=$this->users[$row['usrid']]['status'].$class?>" style="<?=$style?>">
			...
			<div class="message message-col" id="message-<?=$row['pid']?>"><?=$row['message']?></div>
			...
			<div class="message message-col">
				...
				if(!$usrid){
					...
					<form action="/forums/action.php" method="post" data-formaction="<?=($newtopic ? "newtopic" : "reply")?>">
						...
						?>
						<div class="message-compose inpfw">
							<div id="wmd-button-bar" style="height:1px; overflow:hidden;"></div>
							<textarea name="message" id="wmd-input" class="tagging wmd-input autosize forum-reply-message" tabindex="1" placeholder="Your reply here" onchange="confirm_exit=true;" onfocus="$('#wmd-button-bar').animate({height:'25px'}, 100);"><?=$textinp?></textarea>
						</div>
						<div class="message-preview message-col"></div>
						...
						<div class="buttons" style="text-align:right; margin:0 0 0 200px;">
							<button type="button" tabindex="2" id="fmsg-<?=$i?>-prevbutton" onclick="togglePreview(<?=$i?>);">Preview</button> 
							<button type="submit" class="submit" tabindex="3" style="font-weight:bold;"><?=($newtopic ? 'Submit' : 'Submit Reply')?></button>
						</div>
	...
	function newTopicForm() {
		...
		<div id="forum" class="topic <?=$this->suggest['type']?>">
			...
			<div id="forum-posts">
				<ol id="fpostslist">
					<li><? $this->outputReplyForm(0, TRUE, "", TRUE); ?></li>
				</ol>
			</div>
		</div>
		<?
