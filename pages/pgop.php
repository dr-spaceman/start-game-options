...
$act = $_GET['action'];
if(!in_array($act, array("add", "rm", "edit"))) $ajax->kill("There was a form error [action]");
$op = $_GET['op'];
if(!in_array($op, array("love", "hate", "collection"))) $ajax->kill("There was a form error [op]");
if(!$title = formatName($_GET['title'])) $ajax->kill("There was a form error [title]");
...
if(!mysqli_num_rows(mysqli_query($GLOBALS['db']['link'], "SELECT * FROM pages WHERE `title` = '".mysqli_real_escape_string($GLOBALS['db']['link'], $title)."' LIMIT 1"))){
	$ajax->kill('<i>'.$title.'</i> is not yet in the games database.<br/><b><a href="/content/Special:new?title='.formatNameUrl($title).'">Add it to the database</a></b>');
}
...
if ($op == "collection") {
	...
	if(!$ajax->ret['formatted'] = collectionForm($title)) $ajax->kill("Couldn't fetch collection form :( [error location: PGOP]");
