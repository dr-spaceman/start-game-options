...
if($mark_read) {
	...
	if($usrid != $msg->to) {
		...
		if(!mysql_query($query)) echo "Database Error: Could not mark this message as read. Please <a href=\"/bug.php\">submit a bug report</a> to the staff";
	}
}
...
if($delete_message) {
	...
	if(!$who) {
		...
		if(!mysql_query($query)) $errors[] = "Database Error: Could not delete. Please <a href=\"/bug.php\">submit a bug report</a> to the staff";
...
if($view) {
	...
	if($msg->read == 0 && $usrid == $msg->to) {
		...
		if(!mysql_query($query)) $errors[] = "Database Error: Could not mark this message as read.";
	}
