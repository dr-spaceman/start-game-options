...
if($edid = $_GET['edit']) {
	...
	$in = mysqli_fetch_assoc(mysqli_query($GLOBALS['db']['link'], $q));
	if(!$in) $page->kill(404);
	$page->title.= " / Edit Post / " . $in['description'];
...
if($_GET['autotag'] && $_GET['instruct'] == "albumsynopsis") {
	$warnings[] = "For your post item to be featured as the synopsis on the album page, use the following settings: <a href=\"#?field=postsection\">Post to</a> <b>Public Stream</b>; check <b>Archive Post</b>";
}
...
?>
<h1 style="float:left; margin:0; padding:0;"><?=($edid ? 'Edit Post <a href="'.$post->url.'">#'.$edid.'</a>' : 'New Post')?></h1>

<?
$user = new user($usrid);
if($user->notfound) $page->kill('<br style="clear:both"/><big style="font-size:22px;">Please <b><a href="/login.php">Log in</a></b> to continue.</big><p style="font-size:14px;">Don\'t have an account? <b><a href="/register.php">Register</a></b> in about one minute.</p>');
...
<div style="text-align:right; margin:0 0 0 400px">
	...
	<span style="color:#CCC">
		...
		<a href="/formatting-help">Formatting your article</a>
	</span>
</div>
			...
			<div id="inpcontent" class="viewspace view-edit">
				...
				<div id="ttype" class="selection">
					<ul>
						...
						<li><a href="#summary" title="A summary or synopsis of a game, album, etc." class="tooltip">Summary</a></li>
				...
				<div class="main">
					...
					<div class="inpfw" style="margin:20px 0 0">
						<textarea name="cont[text]" rows="2" id="inp-text" class="tagging autosize wmd-input" onfocus="$('#inp-text-toolbar').animate({height:'25px'});"><?=$cont['text']?></textarea>
					</div>
					...
					<!--subject-->
					<div class="ttype ttype-review ttype-preview ttype-playlog ttype-summary inpfw fftt" style="display:none; margin:20px 0 0">
						...
						<label class="tt">
							...
							<span class="ttype ttype-summary">What are you summarizing?</span>
						</label>
					</div>
					<div id="subject-details" class="ttype ttype-review ttype-preview ttype-playlog ttype-summary"></div>
				...
				<div id="appendcontent">
					...
					<div class="forms form-video inpfw fftt">
						<textarea name="in[video_input]" rows="2" id="inpembedcode" class="ff"><?
					...
					<div class="forms form-audio">
						...
						$audio_filename = '';
						if($cont['audio_file']){
							$audio_filename = substr($cont['audio_file'], strrpos($cont['audio_file'], "/") + 1);
						}
						?>
					...
					<div class="forms form-tweet inpfw" style="margin-top:10px">
						...
						<input type="text" name="cont[tweet_url]" value="<?=($cont['tweet']['url'] ? htmlSC($cont['tweet']['url']) : 'http://')?>" maxlength="255" style="font-size:18px; font-family:Arial; color:blue; text-decoration:underline;" onfocus="if($(this).val()=='http://') $(this).val('');" onblur="if($(this).val()=='') $(this).val('http://');"/>
					</div>
		...
		<div id="formfooter">
			...
			<div id="postsection">
				<dl>
					<dt>Post to</dt>
					<?
					if($in['category'] == "draft" && isset($in['options']['on_comeback_category'])){
						$in['category'] = $in['options']['on_comeback_category'];
					}
					?>
					<!--draft-->
					<dd>
					...
					<dd>
						...
						<p>Obective posts that the gaming community will find interesting or important. These posts see significantly more exposure than Blogs.</p>
						...
						<p><label><input type="checkbox" name="archive" value="1"<?=($in['archive'] ? ' checked="checked"' : '')?> id="archive-ch" onclick="$('#archive-sel').val($(this).is(':checked') ? '1' : '')"/> <b>Archive Post</b></label> <a href="#" class="tooltip helpinfo" title="Archive posts are supplementary content meant to be displayed on related database archives (game pages, people pages, etc). Archive posts aren't displayed on the home page or other post lists. Use this option when posting supplementary content that isn't newsworthy."><span>?</span></a></p>
					</dd>
				</dl>
			...
			<div id="postshare">
				...
				//Remove Twitter option if there's no token & secret
				if(!$oauth['twitter']['oauth_token'] && $oauth['twitter']['oauth_token_secret']) unset($oauth['twitter']);
				
				$twitter_disabled = $in['options']['tweet_id'] || !$oauth['twitter'] ? 'disabled="disabled"' : '';
				$twitter_checked = !$twitter_disabled && $user->preferences->twitter_sblog_auth ? 'checked="checked"' : '';
				$facebook_disabled = $in['options']['facebook_post_id'] || !$oauth['facebook'] ? 'disabled="disabled"' : '';
				$facebook_checked = !$facebook_disabled && $user->preferences->fb_sblog_auth ? 'checked="checked"' : '';
				
				?>
				<dl>
					<dt>Share</dt>
					<? if($newpost || $usrid == $postdat['usrid']){ ?>
					<dd>
						<input type="checkbox" name="share[facebook]" value="facebook" id="share-facebook" <?=$facebook_disabled?> <?=$facebook_checked?>/> 
						<label for="share-facebook">Post on my Facebook timeline</label> 
						<a onclick="nnInitOauth('facebook')">Authorize Facebook access</a>
					</dd>
					<dd>
						<input type="checkbox" name="share[twitter]" value="twitter" id="share-twitter" <?=$twitter_disabled?> <?=$twitter_checked?>/> 
						<label for="share-twitter">Tweet<?=($oauth['twitter'] ? ' <span class="a">@'.$oauth['twitter']['oauth_username'].'</span>' : '')?></label> 
			...
			<div id="postopts">
				...
				<div style="display:none; margin:0 0 0 20px; max-width:500px;">
				...
				<dl style="display:none; margin:0 0 0 20px;">
					...
					<dd>
						<span style="font:normal 12px monospace;">
							http://videogam.in/sblog/<span id="permalink-nid"><?=($nid ? $nid : '123456')?></span>/<input type="text" name="in[permalink]" value="<?=$in['permalink']?>" size="60" maxlength="100" id="inp-permalink" class="trackchange" style="margin:0 !important; padding:0; font:normal 12px monospace; border-width:0 0 1px 0 !important; border-color:#999; background-color:transparent;"/>
						</span>
					</dd>
					<? } ?>
					...
					<dd><textarea name="in[description]" rows="2" cols="50" id="inp-description" class="trackchange"><?=$in['description']?></textarea></dd>
					...
					<dt>Post Lists</dt>
					<dd>
						<select name="in[archive]" id="archive-sel" onchange="$('#archive-ch').prop('checked',($(this).val() ? true : false))">
							<option value="">Viewable on all related post lists</option>
							<option value="1">Archive &ndash; Viewable only on related archives</option>
						</select> &nbsp; <a href="#" class="tooltip helpinfo" title="Archive posts are supplementary content meant to be displayed on related database archives (game pages, people pages, etc). Archive posts aren't displayed on the home page or other post lists. Use this option when posting supplementary content that isn't newsworthy."><span>?</span></a>
					</dd>
					<dd>
					<dd>
						<label><input type="checkbox" name="in[minimize]" value="1" <?=($in['minimize'] ? 'checked="checked"' : '')?>/> <b>Minimize</b> &ndash; Only show a simple post description and a link to the full article</label>
					</dd>
				...
				if($_SESSION['user_rank'] >= 8){
					...
					<dl style="display:none">
						...
						foreach($in as $k => $v){
							...
							if(is_array($v)) continue;//print_r($v);
							else echo htmlentities($v);
							echo '</dd>';
							//if($k == "content"){ echo '<dd style="white-space:pre-wrap; word-break:break-all;"><hr/>'; print_r($cont); echo '</dd>'; }
						}
						?>
					</dl>
					<?
